...
- [x] **New CLI Options**: Add arguments to control history scanning:
    - `--scan-history`: Flag to enable checking every commit.
- [x] **Metric Limits**: Implement limits for the scan (by default, no limits):
...
- [x] **Performance**: Ensure that checking historical commits does not drastically reduce performance. Optimize by avoiding full working tree checkouts and instead piping file blobs directly to the scanner.

## 4. Testing & Verification (Mandatory)
### 4.1 Unit Testing
- [x] Add unit tests in `tests/test_git_engine.py` to verify that all commits in a mock repository's history are traversed when the flag is enabled.
- [x] Verify that the limit options correctly halt traversal.

### 4.2 Acceptance Testing (BDD)
- [x] Add a new `Scenario` to `tests/acceptance.feature` describing a secret added in commit A and removed in commit B.
- [x] Verify that the scanner finds the secret when history scanning is enabled, but not when disabled.

### 4.3 Test Data Obfuscation
- [x] Ensure any mock git repositories created for testing use fake, obfuscated secrets to prevent triggering real security alerts.

## 5. Demo & Documentation
- [x] **`demo.sh`**: Add a demonstration of scanning a repository's history and finding a "deleted" secret.
- [x] **`README.md`**: Document the `--scan-history` and `--limit-*` flags under the usage instructions.
- [x] **CLI Help**: Verify the new flags appear correctly in `secret-scan --help` with appropriate default descriptions (e.g., "by default no limit").
